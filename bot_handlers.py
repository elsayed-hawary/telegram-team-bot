...
async def _send(chat_id: int, ctx: ContextTypes.DEFAULT_TYPE, text: str):
    # دايمًا نبعث الكيبورد الكبيرة مع كل رسالة
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _send(update.effective_chat.id, context,
        ...
        "ابعث /start أو أي زر لتظهر الكيبورد إن ما ظهرتش.")

async def cmd_about(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _track_user(update)
    await _send(update.effective_chat.id, context, "منيو سفلية (Reply Keyboard) + إدارة فرق ✅")
...
async def act_set_team(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _track_user(update)
    if OWNER_ID == 0:
        return await _send(update.effective_chat.id, context, "⚠️ ضيف OWNER_ID في Environment أولاً.")
...
async def on_decision(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "APPROVE":
        ...
        deny(team_name, uid); await q.edit_message_text(f"✖️ تم رفض طلب {uid} لـ {team_name}.")
